window.onload=function(){
    ...
    var draw = function(ch, cls) {
        ...
        for(var y = 0; y < f.length; y++)
            for(var x = 0; x < f[y].length; x++)
                if(f[y][x] === '1') {
                    if(x + pos[0] + ch[0] >9 || x + pos[0] + ch[0] < 0 || y + pos[1] + ch[1] > 19 || gP(x + pos[0] + ch[0], y + pos[1] + ch[1]).classList.contains('on'))
                        return false;
    ...
    window.addEventListener('keydown', kdf = function(e) {
        if(e.keyCode === 38 && false !== (now[1] = ((prv = now[1]) + 1) % fs.split('*')[now[0]].split(':').length) && false === draw([0,0], undefined, deDraw()))
            draw([0,0], undefined, deDraw(), now = [now[0], prv]);
        if((e.keyCode === 39 || e.keyCode === 37) && false === draw([e.keyCode === 39 ? 1 : -1, 0], undefined, deDraw()))
